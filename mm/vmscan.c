...
int kswapd_run(int nid)
{
	...
	for (hid = 0; hid < nr_threads; hid++) {
		pgdat->kshrinkd[hid] = kthread_run(kshrinkd, pgdat, "kshrinkd%d", nid, hid);
		if (IS_ERR(pgdat->kshrinkd[hid])) {
			/* failure at boot is fatal */
			BUG_ON(system_state < SYSTEM_RUNNING);
			pr_err("Failed to start kshrinkd on node %d\n", nid);
			ret = PTR_ERR(pgdat->kshrinkd[hid]);
			pgdat->kshrinkd[hid] = NULL;
			return ret;
		}
		...
		if (IS_ERR(pgdat->kswapd[hid])) {
			...
			pgdat->kswapd[hid] = NULL;
			kthread_stop(pgdat->kshrinkd[hid]);
			pgdat->kshrinkd[hid] = NULL;
		}
	}
...
void kswapd_stop(int nid)
{
	...
	for (hid = 0; hid < nr_threads; hid++) {
		...
		kshrinkd = NODE_DATA(nid)->kshrinkd[hid];
		if (kswapd) {
			kthread_stop(kswapd);
		...
		if (kshrinkd) {
			...
			NODE_DATA(nid)->kshrinkd[hid] = NULL;
		}
	}
}
