				  ...
				  struct affinity_context *ctx)
{
	...
	if ((p->pc_flags & PC_PRIME_AFFINE) && ctx->new_mask != cpu_prime_mask)
		return -EINVAL;

	if ((p->pc_flags & PC_PERF_AFFINE) && ctx->new_mask != cpu_perf_mask)
		return -EINVAL;

	if ((p->pc_flags & PC_LITTLE_AFFINE) && ctx->new_mask != cpu_lp_mask)
		return -EINVAL;
